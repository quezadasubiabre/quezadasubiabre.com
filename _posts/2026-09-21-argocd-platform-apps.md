...
**networking**: as we'll host some applications in the cluster that will be exposed through the internet, we need:
- Traefik ingress controller
- a load balancer

The GPU-related components (nvidia-device-plugin, dcgm-exporter) are their own topic — I'll cover those in a future post.

Keep in mind this is a developer platform, so some components aren't set up with a production-grade approach.

To explain how this platform was built, I'll go step by step setting up each application.

## Install Argo CD

First we start with the Kubernetes cluster with a single node, the system node with the `t3.medium`. I wanted to use a smaller (cheaper) instance, but it didn't have enough resources for all the applications.

The first step is to log in to the running cluster:

```
aws eks update-kubeconfig --region eu-west-1 --name k8s-cloud-project
```

After this, the cluster credentials will be configured on your computer, so you can access it. To manage the cluster I'm using k9s.

![k9s showing the kube-system pods running on the system node](/assets/img/posts/argocd-platform-apps-k9s-cluster.png)
...
After we're logged in to the cluster, we can install Argo CD. I like keeping every setup step as a `make` target rather than remembering raw commands, so this is driven by a `Makefile`:
...
![Argo CD UI on first login, with no Applications yet](/assets/img/posts/argocd-platform-apps-argocd-init.png)
...
To install an application in Argo CD, I define two things: an Application manifest in the `bootstrap` folder, and its parameters in the `apps` folder, both inside `gitops`.

In `gitops/bootstrap/` each file is an Argo CD `Application` resource — it points at the upstream Helm chart (or a folder of raw manifests) and, via a second source, at this same repo. In `gitops/apps/<name>/` lives the `values.yaml` (or raw YAML) that second source references, which is what actually configures the app once it's deployed.

I'll divide this section into monitoring apps, networking apps, and the first app deployed on top of the platform.
...
![Argo CD UI after deploying storage-class, loki and monitoring apps](/assets/img/posts/argocd-platform-apps-argocd-app-1.png)
...
![Argo CD storage-class](/assets/img/posts/argocd-platform-apps-storage-class-argo-cd.png)
...
![k9s showing the pods deployed by Argo CD](/assets/img/posts/argocd-platform-apps-k9s-1.png)

Grafana doesn't have a public URL yet — same as Argo CD, I reach it through a port-forward, using the `open-grafana` target from the Makefile:
...
Running `make open-grafana` forwards Grafana's Service to my machine, so I can open it at `http://localhost:3000` and log in with the default `admin`/`admin` credentials set in `gitops/apps/monitoring/values.yaml`:

![Grafana's login and home screen](/assets/img/posts/argocd-platform-apps-graphana.png)
...
![Cluster dashboard in Grafana](/assets/img/posts/argocd-platform-apps-graphana-cluster-metrics.png)
...
serviceAccount:
  ...
  annotations:
    ...
    eks.amazonaws.com/role-arn: arn:aws:iam::123456789101112:role/k8s-cloud-project-lbc
```
...
spec:
  project: default
  source:
    repoURL: https://github.com/quezadasubiabre/aws-eks-platform.git
    targetRevision: main
...
![The static site served over HTTPS at demo.quezadasubiabre.com/hello, showing the custom AUTHOR text](/assets/img/posts/argocd-platform-apps-hello-word-app-web.png)
...
![The static-site Application's resource tree in the Argo CD UI, all resources Healthy and Synced](/assets/img/posts/argocd-platform-apps-static-site.png)

With the networking layer proven end to end, the platform is ready for the actual payload. In the next post, I'll cover how to host a GPU workload on this same cluster using the nvidia-device-plugin and dcgm-exporter.
